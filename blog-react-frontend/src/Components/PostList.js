...
export default function PostList() {
  ...
  const addCatgeoryDialog = () => {
    return (
      <div>
        ...
        <Dialog open={categoryDialog} onClose={closeCategoryDialog}>
          <DialogContent>
  ...
  const [getCategoryPoster, setCategoryPoster] = useState({
    bytes: "",
    filename: "",
  });
          ...
          <Grid container spacing={5}>
            <Grid item md={12} className="center">
              <Button
                ...
                onChange={handleCategoryPoster}
                component="label"
              >
                <input type="file" accept="images/*" hidden />
                <Avatar
                  src={getCategoryPoster.filename}
  ...
  const handleDeletePost = async (postID) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetchPostList();
        Swal.fire("Đã xóa!", "Đã xóa bài viết.", "success");
      }
    });
  };
          ...
          <Grid container spacing={5}>
            ...
            <Grid item md={3}>
              ...
              <TextField
                ...
                style={{ marginTop: "10%" }}
              />
              <div style={{ marginTop: "15%" }}>{Category()}</div>
              <Paper
                sx={{
                  display: "flex",
              ...
              <div style={{ marginTop: "10%" }}>
                <img
                  src={getPoster.filename}
                  style={{ width: "100%", borderRadius: "10px" }}
                />
              </div>

              {getBtnStatus ? (
                <Grid container spacing={2} className="left">
  ...
  const newPostDialog = () => {
    return (
      <div>
        <Dialog fullScreen open={createPostDialog}>
          <AppBar sx={{ position: "relative", background: "#004cef" }}>
            <Toolbar>
              <IconButton
                edge="start"
