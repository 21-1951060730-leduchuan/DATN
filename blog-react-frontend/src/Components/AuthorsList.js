...
export default function AuthorsList() {
    ...
    const handleUpdateAuthorData = async () => {
        Swal.fire({
            title: 'Bạn có chắc thay đổi tác giả này?',
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                ...
                fetchAuthor()
                Swal.fire('Đã sửa!', '', 'success')
            } else if (result.isDenied) {
                Swal.fire('Không thay đổi', '', 'info')
            }
        })
    }
                ...
                <Grid container spacing={0} className='center'>
                    ...
                    <Grid container spacing={3} style={{ marginTop: '5%' }}>
                        <Grid item md={12}>
                            <TextField
                                ...
                                fullWidth onChange={(event) => setAuthorName(event.target.value)} label='Tên' />
                        </Grid>
                    ...
                    <Grid container spacing={3} style={{ display: 'flex', justifyContent: 'right', alignItems: 'center', marginTop: '3%' }}>
                        <Grid item md={5} style={{ display: 'flex', justifyContent: 'right', alignItems: 'center' }}>
                            <Button startIcon={<AddCircleOutlineIcon />} onClick={handleAddAuthor} variant='outlined' style={{ marginRight: '2%', borderColor: '#0069ff', color: '#0069ff' }}>Thêm</Button>
                        </Grid>
                    </Grid>
                </Grid>
            </div>
            ...
            <div>
                <Modal
                    ...
                    <Box sx={style}>
                        ...
                        <Grid container spacing={3} style={{ marginTop: '1%' }}>
                            <Grid item md={12}>
                                <TextField
                                    ...
                                    error={getErrors.getAuthorName}
                                    helperText={getErrors.getAuthorName}
                                    fullWidth value={getAuthorName} onChange={(event) => setAuthorName(event.target.value)} label='Tên' />
                            </Grid>
                        ...
                        <Grid container spacing={3} style={{ display: 'flex', justifyContent: 'right', alignItems: 'center', marginTop: '3%' }}>
                            <Grid item md={5} style={{ display: 'flex', justifyContent: 'right', alignItems: 'center' }}>
                                <Button startIcon={<SaveIcon />} onClick={handleUpdateAuthorData} variant='outlined' style={{ marginRight: '2%', borderColor: '#0069ff', color: '#0069ff' }}>Sửa</Button>
                                <Button startIcon={<DeleteIcon />} onClick={() => handleDeleteAuthor(getAuthorID)} variant='outlined' style={{ marginLeft: '4%', borderColor: 'red', color: 'red' }}>Xóa</Button>
                            </Grid>
                        </Grid>
                    </Box>
                </Modal>
        ...
        return (
            <Grid container spacing={1} style={{ padding: '3% 10% 3% 7%', margin: 0 }}>
                <Grid container spacing={1} style={{ display: 'flex', alignItems: 'center', marginBottom: '4%' }}>
                    <Grid item xs={6}>
                        <span style={{ fontWeight: '600', fontSize: '25px' }}>Tác giả</span>
                    </Grid>
                </Grid>
                <Grid
                    ...
                    return (
                        <Grid item md={3} style={{
                            ...
                            <center>
                                ...
                                <Typography style={{ fontWeight: 500, fontSize: '17px', opacity: '70%' }}>Số bài viết - 5</Typography>
                            </center>
                        </Grid>
                    )
                })}
